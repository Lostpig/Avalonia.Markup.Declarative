...
internal class Program
{
    static async Task Main(string[] args)
    {
        ...
        if (args is ["--framework"])
        {
            ...
            projectPath = Path.Combine(projectDir, "Mearii.Mvu.csproj");
        }

        if (projectPath == null)
        {
            Console.WriteLine("Project file not found. Please provide the path to the .csproj file using the argument '--projectPath' or run Mearii.AvaloniaExtensionGenerator from directory that contains csproj");
            return;
        }
